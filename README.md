...
- [x] Real team info in `config/team_info.json` (see below).
- [x] Re-registered at LLMod.ai with the course-required email before requesting the project key.
- [x] `LLMOD_API_KEY`/`LLMOD_MODEL` set in local `.env` (never committed).
- [ ] **Submission deadline: 23 August 2026.**
- [ ] **Total LLM budget: $13/group** — `MAX_PROJECT_BUDGET_USD=13` in `.env.example`.
- [x] `pytest -q` and `ruff check .` pass with `MOCK_LLM=true` (the default, $0).
- [x] Deployed on Vercel (required by the spec — see "Deployment").
...
Full diagram: `assets/model_architecture.png` (also `GET /api/model_architecture`). Module names are
pulled from `app/core/module_names.py` everywhere (code, diagram, `/api/agent_info`, LLM tracing) —
...
Shape: `{"description", "purpose", "prompt_template": {"template"}, "prompt_examples": [{"prompt",
"full_response", "steps": [{"module", "prompt", "response"}]}]}`. `prompt_examples` are **real
answers** captured from live runs (`assets/prompt_examples.json`, exported by
`scripts/export_prompt_examples.py`) — real Markdown, real `System_prompt`/`User_prompt` per step,
not summaries. File read only, no LLM/network call; see the live endpoint for the actual content.

### `GET /api/model_architecture`

Raw PNG bytes, `Content-Type: image/png` — not JSON, not Base64. Generated by
`scripts/render_architecture.py` from the same module/tool names as the code
...
Hard backend deadline **270s** (research stops at 210s, keeping completed results; 60s left to score
and generate), inside the course's 300s Vercel ceiling — always disclosed on fallback, never a raw
error or a hang. The request body is always exactly `{"prompt": "..."}`, including for automated
grading — it always resolves to one final recommendation, never stopping to ask a clarifying
question (the would-be question becomes a disclosed assumption instead). The deployed frontend is
the one exception: it adds an `X-Interactive-Mode: true` **header** (never a body field) to restore
real clarification questions for a human at the keyboard.

`DELETE /api/history` and `POST /api/history/delete` are additive UI-only endpoints (sidebar
clear-history), not part of the 4 required endpoints and not used by the agent pipeline.
...
## Docker (optional — Vercel is what's graded)
...
Defaults to `MOCK_LLM=true`. `SQLITE_PATH` can be volume-mounted for persistence
(`-v ${PWD}/data:/app/data`).

## Deployment

The course requires **Vercel** specifically (`docs/Project instructions.pdf`). `vercel.json`
configures: `main.py`'s `app` as the entrypoint (Vercel auto-detects it, no wrapper needed);
`routes` sending every path to it; `functions.main.py.maxDuration: 300` (our `270`s deadline fits
with margin); `env.MOCK_LLM: "false"` (the real provider is graded, unlike Docker's `true` default)
and `SQLITE_PATH=/tmp/...` (Vercel's filesystem is read-only except `/tmp`, which resets every cold
start — see "Storage" below).
